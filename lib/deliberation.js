  ...
  async deliberate(prompt, options = {}) {
    ...
    const panelPromises = [
      ...
      this.api.chatCompletion({
        ...
        temperature: 0.5,
      }).then(res => ({ persona: 'technical_expert', content: res })),
      ...
      this.api.chatCompletion({
        ...
        temperature: 0.8,
      }).then(res => ({ persona: 'devils_advocate', content: res })),
      ...
      this.api.chatCompletion({
        ...
        temperature: 0.6,
      }).then(res => ({ persona: 'systems_thinker', content: res }))
    ];
    ...
    const panelResponses = {
      technical_expert: panelResults.find(r => r.persona === 'technical_expert').content,
      devils_advocate: panelResults.find(r => r.persona === 'devils_advocate').content,
      systems_thinker: panelResults.find(r => r.persona === 'systems_thinker').content,
    };
